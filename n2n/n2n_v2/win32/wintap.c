...
int open_wintap(struct tuntap_dev *device,
  ...
  //Close the handle so we can update the MAC address in the registry
  CloseHandle(device->device_handle);

  /* Open registry and look for the network adapter in a separate key 
	 based on the adapterid we just found 
	*/
  if ((rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, ADAPTER_KEY, 0, KEY_READ, &key3))) {
	  printf("Unable to read registry: [rc=%d]\n", rc);
	  exit(-1);
  }

  char adapterid2[1024];
  char NetCfgInstanceId[1024];
  for (i = 0; ; i++) {
	  len = sizeof(adapterid2);
	  if (RegEnumKeyEx(key3, i, (LPTSTR)adapterid2, &len, 0, 0, 0, NULL))
		  break;


	  _snprintf(regpath, sizeof(regpath), "%s\\%s", ADAPTER_KEY, adapterid2);
	  if ((rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, regpath, 0, KEY_READ, &adapterKey))) {
		  printf("Unable to read registry: [rc=%d]\n", rc);
		  exit(-1);
	  }

	  len = sizeof(NetCfgInstanceId);
	  err = RegQueryValueEx(adapterKey, "NetCfgInstanceId", 0, 0, NetCfgInstanceId, &len);
	  
	  if (err)
		  continue;

	  if (!strcmp(NetCfgInstanceId, adapterid)) { //We have a match, let's set the MAC
		  //Close it and re-open with write access
		  RegCloseKey(adapterKey);
		  _snprintf(regpath, sizeof(regpath), "%s\\%s", ADAPTER_KEY, adapterid2);
		  if ((rc = RegOpenKeyEx(HKEY_LOCAL_MACHINE, regpath, 0, KEY_ALL_ACCESS, &adapterKey))) {
			  
			  printf("Unable to write registry MAC address: [rc=%d]\n", rc);
			  exit(-1);
		  }
		  
		  LONG setRes = RegSetValueEx(adapterKey, "MAC", 0, REG_SZ, (LPBYTE)device_mac, (wcslen(device_mac) +1) * sizeof(WCHAR));
  ...
  //Enable and disable the adapter to make it pick up the MAC change
  char netshString[1024];
  _snprintf(netshString, sizeof(netshString), "netsh interface set interface \"%s\" disabled\n", device->ifName);
  //printf(netshString);
  system(netshString);
  //Sleep(5000);

  _snprintf(netshString, sizeof(netshString), "netsh interface set interface \"%s\" enabled\n", device->ifName);
  //printf(netshString);
  system(netshString);
